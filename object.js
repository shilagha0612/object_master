...
const typePokemon = pokémon.filter(p => p.types.includes("fire"))
console.log(typePokemon)

morePokémon = pokémon.filter(p => p.types.length > 1)
console.log(morePokémon)

onlyPokémon = pokémon.map(p => p.name)
console.log(onlyPokémon)

const greaterThan = pokémon.filter(p => p.id > 99).map(p => p.name)
console.log(greaterThan)

const typePoison = pokémon.filter(p => p.types == 'poison').map(p => p.name)
console.log(typePoison)

const namesWithOnlyFlyingType = pokémon
